<x-app-layout>
    ...
    <div class="relative">
        <div
            ...
            <form action="{{ route('admin.pengembalian.update', $data->id)}}" method="POST">
                @csrf
                <div class="grid grid-cols-2 gap-3">

                    <div class="mb-2">
                        <x-input-label for="field-pemesanan_id" value="Pemesanan" />
                        <x-select-field id="pemesanan_id" name="pemesanan_id" placeholder="Pilih" value="{{ $data->pemesanan_id }}"
                            :options="$pemesanan" />
                        <x-input-error :messages="$errors->get('pemesanan_id')" class="mt-2" />
                    </div>
                    <div class="mb-2">
                        <x-input-label for="field-tgl" value="Tanggal Kembali" />
                        <x-text-input id="field-tgl" class="block mt-1 w-full" type="text" name="tgl" value="{{ $data->tgl }}"
                            :value="old('tgl')" />
                        <x-input-error :messages="$errors->get('tgl')" class="mt-2" />
                    </div>
                    <div class="mb-2">
                        <x-input-label value="Keterlambatan" />
                        <div id="showLambat">{{ $data->telat }} Hari</div>
                        <input type="hidden" id="field-lambat" name="lambat" value="{{ $data->telat }}"/>
                        <x-input-error :messages="$errors->get('lama')" class="mt-2" />
                    </div>
                    <div class="mb-2">
                        <x-input-label for="field-status" value="status" />
                        <x-select-field id="status" name="status" placeholder="Pilih" value="{{ $data->status }}" :options="[
                    ...
                    <tbody class="divide-y divide-gray-300">
                        ...
                        @endphp
                            ...
                            <tr class="row-${idx}">
                                ...
                                <td class="px-3 py-2 whitespace-nowrap text-gray-800 dark:text-neutral-200">
                                    <x-text-input id="field-rusak_ringan" class="block mt-1 w-full line-rusak_ringan"
                                        type="number" min="0" value="{{ $d->rusak_ringan }}" name="lines[${idx}][rusak_ringan]" />
                                </td>
                                <td class="px-3 py-2 whitespace-nowrap text-gray-800 dark:text-neutral-200">
                                    <x-text-input id="field-rusak_sedang" class="block mt-1 w-full line-rusak_sedang"
                                        type="number" min="0" value="{{ $d->rusak_sedang }}" name="lines[${idx}][rusak_sedang]" />
                                </td>
                                <td class="px-3 py-2 whitespace-nowrap text-gray-800 dark:text-neutral-200">
                                    <x-text-input id="field-rusak_total" class="block mt-1 w-full line-rusak_total"
                                        type="number" min="0" value="{{ $d->rusak_total }}" name="lines[${idx}][rusak_total]" />
                                </td>
                                <td class="px-3 py-2 whitespace-nowrap text-gray-800 dark:text-neutral-200">
